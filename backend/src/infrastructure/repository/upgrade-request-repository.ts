import {
  IUpgradeRequest,
  UpgradeRequestModel,
} from "../db/model/upgrade-request-model";
...
export class UpgradeRequestRepository {
  ...
  async create(
    data: Omit<IUpgradeRequest, "_id" | "createdAt" | "updatedAt" | "status">,
  ): Promise<IUpgradeRequest> {
    const doc = await UpgradeRequestModel.create({
      ...data,
      status: "pending",
    });
  ...
  async update(
    id: string,
    data: Partial<IUpgradeRequest>,
  ): Promise<IUpgradeRequest | null> {
    const doc = await UpgradeRequestModel.findByIdAndUpdate(
      id,
      { $set: data },
      { new: true },
    )
