...
export function startRentReminderCron() {
  ...
  cron.schedule("0 8 * * *", async () => {
    ...
    try {
      const activeTenants = await tenantRepo.findAll({
        status: "active",
      } as any);
      ...
      for (const tenant of activeTenants) {
        ...
        if (send) {
          // Notify the Tenant
          await emailService
            .sendNotificationEmail(tenant.email, title, message)
  ...
  logger.info(
    "[Cron] Rent Reminder Cron Job safely registered (Target: daily 08:00 AM).",
  );
}
