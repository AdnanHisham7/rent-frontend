    ...
    try {
      if (!req.file)
        throw new BadRequestError(
          "No file uploaded.",
          'Attach a file under the "file" field.',
        );
      ...
      const result = await cloudinaryService.upload(
        req.file.buffer,
        category,
        uniqueName(req.file.originalname),
      );
  ...
  uploadMultiple = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      if (!files.length)
        throw new BadRequestError(
          "No files uploaded.",
          'Attach one or more files under the "files" field.',
        );
      const category = (req.params.category || "misc")
        .toString()
        .replace(/[^a-z0-9_-]/gi, "");
      const uploads = await Promise.all(
        files.map((f) =>
          cloudinaryService.upload(
            f.buffer,
            category,
            uniqueName(f.originalname),
          ),
        ),
      );
      const data = uploads.map((r, i) => ({
        url: r.url,
        publicId: r.publicId,
        size: r.bytes,
      ...
      return res
        .status(201)
        .json({ message: `${files.length} file(s) uploaded.`, data });
    } catch (error: any) {
      return res
        .status(error.statusCode || 400)
        .json({ message: error.message || "Upload failed." });
