...
export class SubscriptionController {
  ...
  getMine = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      return this.handleError(res, error, "Failed to fetch subscription.");
    }
  };
  ...
  requestUpgrade = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      return this.handleError(res, error, "Failed to submit upgrade request.");
    }
  };
  ...
  bookDemo = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      return this.handleError(res, error, "Failed to submit demo request.");
    }
  };

  // ── Admin endpoints ──────────────────────────────────────────────────────────

  createBuilderSubscription = async (
    req: Request,
    res: Response,
  ): Promise<Response> => {
    try {
      ...
      return res
        .status(201)
        .json({ message: "Subscription created successfully.", ...result });
    } catch (error) {
      return this.handleError(res, error, "Failed to create subscription.");
    }
  };
  ...
  adminUpdate = async (
    ...
    try {
      const data = await this.uc.adminUpdate(
        req.params.id,
        req.body,
        req.user!.userId,
      );
  ...
  markPeriodPaid = async (
    ...
    try {
      const data = await this.uc.markPeriodPaid(
        req.params.periodId,
        req.user!.userId,
        req.body,
      );
      return res
        .status(200)
        .json({ message: "Period marked as paid. Next period created.", data });
    } catch (error) {
      return this.handleError(res, error, "Failed to mark period as paid.");
    }
  };
  ...
  listPeriods = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const { userId, subscriptionId, status } = req.query as Record<
        string,
        string
      >;
      const result = await this.uc.listPeriods(
      ...
      return this.handleError(res, error, "Failed to list periods.");
    }
  };
  ...
  listDemoRequests = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      return this.handleError(res, error, "Failed to list demo requests.");
    }
  };
  ...
  updateDemoRequest = async (
    ...
    try {
      ...
      return this.handleError(res, error, "Failed to update demo request.");
    }
  };
}
