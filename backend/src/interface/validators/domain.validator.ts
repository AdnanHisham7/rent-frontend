...
export const createExpenseSchema = z.object({
  ...
  method: z.enum(expenseMethods, {
    message: `method must be one of: ${expenseMethods.join(", ")}.`,
  }),
...
export const createFloorSchema = z.object({
  floorNumber: z.coerce.number({
    message: "floorNumber is required and must be a number.",
  }),
...
export const createDocumentSchema = z.object({
  type: z.enum(documentTypes, {
    message: `type must be one of: ${documentTypes.join(", ")}.`,
  }),
  title: z.string().trim().min(1, "title is required."),
...
export const bookDemoSchema = z.object({
  ...
  numberOfBuildings: z.coerce
    .number()
    .int()
    .min(1, "numberOfBuildings must be at least 1."),
  numberOfUnits: z.coerce
    .number()
    .int()
    .min(1, "numberOfUnits must be at least 1."),
...
export const requestUpgradeSchema = z.object({
  ...
  additionalBuildingData: z
    .array(z.object({ name: z.string(), rooms: z.number() }))
    .optional(),
  message: z.string().trim().optional(),
});
...
export const resolveUpgradeRequestSchema = z.discriminatedUnion("status", [
  ...
  z.object({
    ...
    adminNotes: z.string().trim().optional(),
    newTotalBuildings: z.coerce
      .number()
      .int()
      .min(1, "newTotalBuildings must be at least 1."),
    newTotalUnits: z.coerce
      .number()
      .int()
      .min(1, "newTotalUnits must be at least 1."),
  }),
]);
