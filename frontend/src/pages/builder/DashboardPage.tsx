        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between border-b border-line px-5 py-4">
              ...
              <button
                onClick={onClose}
                className="rounded-lg p-1.5 text-ink-faint hover:bg-paper-dim hover:text-ink"
              >
                <X className="size-4" />
              </button>
            </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-line">
                    {transactions.map((t) => (
                      <tr
                        key={t._id}
                        className="hover:bg-paper-dim/50 transition-colors"
                      >
                        ...
                        <td className="px-4 py-3 text-right font-mono font-semibold text-sage-700">
    ...
    >
      <Card
        ...
        className={cn(
          ...
          clickable &&
            "cursor-pointer hover:shadow-md hover:border-crimson-300 transition-shadow",
        )}
        onClick={onClick}
      >
        <div>
        ...
        <span className="flex size-10 items-center justify-center rounded-xl bg-crimson-50 text-crimson-500">
          {icon}
        </span>
      </Card>
    </motion.div>
...
export default function DashboardPage() {
  ...
  const { data: trendsData } = useGetRoomTrendsQuery();
  ...
  const pieData = occ
    ? [
        { name: "Occupied", value: occ.occupiedUnits },
        { name: "Vacant", value: occ.vacantUnits },
      ]
    : [];
  ...
  return (
    <div>
      ...
      {(metrics?.pendingPayments?.length ?? 0) > 0 && (
        ...
        >
          <p className="text-sm font-medium text-amber-800">
            {metrics!.pendingPayments.length} tenant
            {metrics!.pendingPayments.length !== 1 ? "s have" : " has"} pending
            payments
          </p>
          <div className="mt-1.5 flex flex-wrap gap-2">
            {metrics!.pendingPayments.slice(0, 5).map((p) => (
              <span
                key={p.tenantId}
                className="rounded-full bg-amber-100 px-2.5 py-0.5 text-[11px] text-amber-800"
              >
        ...
        <Card className="lg:col-span-2" padding="lg">
          ...
          {chartData.length === 0 ? (
            <EmptyState
              ...
              description="Once payments are recorded, you'll see building-wise revenue here."
            />
          ) : (
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="flex gap-4 text-xs">
                <span className="flex items-center gap-1.5">
                  <span className="size-2 rounded-full bg-crimson-500" />{" "}
                  Occupied
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="size-2 rounded-full bg-paper-deep" /> Vacant
                </span>
              </div>
            </div>
          )}
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <Card padding="lg">
          <h3 className="mb-4 flex items-center gap-2 font-display text-base font-semibold text-ink">
            <FileWarning className="size-4 text-amber-500" /> Expiring
            agreements
          </h3>
          {!metrics?.expiringAgreements.length ? (
            ...
            <div className="flex flex-col divide-y divide-line">
              {metrics.expiringAgreements.slice(0, 5).map((a) => (
                <Link
                  key={a.agreementId}
                  to={`/dashboard/agreements/${a.agreementId}`}
                  className="flex items-center justify-between py-2.5 text-sm hover:text-crimson-600"
                >
                  <span className="text-ink-soft">
                    Ends {formatDate(a.endDate)}
                  </span>
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-3">
        <Card className="lg:col-span-2" padding="lg">
          <h3 className="mb-4 flex items-center gap-2 font-display text-base font-semibold text-ink">
            <CalendarRange className="size-4 text-crimson-500" /> 6-month trend
          </h3>
          {!trends?.monthlyTrend.length ? (
            <EmptyState
              title="Not enough data yet"
              description="Revenue and booking trends will appear here as activity comes in."
            />
          ) : (
            <ResponsiveContainer width="100%" height={260}>
        ...
        <Card padding="lg">
          <h3 className="mb-4 flex items-center gap-2 font-display text-base font-semibold text-ink">
            <BedDouble className="size-4 text-crimson-500" /> Room type demand
          </h3>
          {!trends?.roomTypeDemand.length ? (
            <EmptyState title="No rooms yet" />
          ) : (
            <div className="flex flex-col divide-y divide-line">
              {trends.roomTypeDemand.map((r) => (
                <div key={r.bedrooms} className="py-2.5">
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-medium text-ink">{r.bedrooms}BR</span>
                    <span className="text-xs text-ink-faint">
                      {r.occupancyRatePercentage}% occupied
                    </span>
                  </div>
                  <div className="mt-1 flex items-center gap-3 text-xs text-ink-soft">
                    <span>{r.totalUnits} rooms</span>
                    <span>{r.totalBookings} bookings</span>
                    <span>{r.totalInquiries} inquiries</span>
                  </div>
                </div>
              ))}
            </div>
